...
test.describe('Accented', () => {
  ...
  test.describe('issue dialogs', () => {
    ...
    test('the dialog itself doesn’t have accessibility issues identifiable by axe-core', async ({
      page,
    }) => {
      await page.goto('/');
      const dialog = await openAccentedDialog(page, '#various-impacts');
      const violations = await dialog.evaluate(
        async (dialogElement) => (await axe.run(dialogElement)).violations,
      );
  ...
  test.describe('web platform support', () => {
    test.describe('shadow DOM', () => {
      test('an element in shadow DOM has a trigger that is aligned with the element, and an outline', async ({
        page,
      }) => {
        await page.goto('/');
