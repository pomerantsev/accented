...
from mostly static pages to full-blown single-page applications (SPAs).
...
- There are **on-demand auditing tools**, such as browser extensions or [Lighthouse]((https://developer.chrome.com/docs/lighthouse)).
  ...
  such audits are likely performed irregularly and infrequently.
...
- There’s also **[@axe-core/react](https://github.com/dequelabs/axe-core-npm/blob/b374159a9e63c4dad0135fb2353163ffe8c04453/packages/react/README.md)**,
  ...
  Unfortunately, it’s only meant for React applications, and most importantly,
  it doesn’t support modern versions of React,
  and overall it’s in maintenance mode, with no further development plans.

### Comparison table

Here’s a summary table to help you choose the right tools.
...
We strive to minimize its impact:
for instance, we use shadow DOM to isolate the styles.

Still, some changes are unavoidable,
so whenever there’s an accessibility issue on the page,
there is a chance that when Accented makes its modifications,
something else on the page may change.
...
and the strategy here is to use the [`axeOptions`](/api#axeoptions)
and [`context`](/api#context) options
...
and that may be an expensive operation.
