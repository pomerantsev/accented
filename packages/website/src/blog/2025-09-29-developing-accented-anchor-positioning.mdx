...
description: The new CSS anchor positioning feature help Accented align its UI elements more easily than it was possible with absolute or fixed positioning.
...
- as a bonus, I'll touch a bit on [CSS logical properties and values](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_logical_properties_and_values)
  and how they help with positioning as well.
...
- The element can be large or small, be vertically or horizontally aligned.
...
with such a setup, trigger buttons on elements inside a modal dialog (I mean actual [`<dialog>` elements](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/dialog)) simply wouldn’t work.
...
This may add a bit of scroll jank, but it’s likely not that big of a deal for a development tool.

Importantly, this solves the challenges that we mentioned for absolute positioning.

The calculations become easier with fixed positioning: a simple `getBoundingClientRect()` gives us the necessary offsets and sizes.
...
All in all, both options (absolute and fixed positioning) could work, but the latter seems to be a bit easier to work with.
...
And it’s a big deal for Accented, because it allows us (in the future) to get rid of a bunch of JavaScript, and replace it with just a few lines of CSS.
...
- **It’s not supported in all browsers yet (as of September 2025).**
  ...
  and [in the latest Safari (version 26.0) the implementation is buggy](https://github.com/Fyrd/caniuse/pull/7387).
...
- **There’s a subtle difference between `position: fixed` and `position: absolute` when using anchor positioning.**
  ...
  In that case, absolute positioning doesn’t work as I would expect it to,
  and the relevant specs don’t clarify this difference in behavior.
...
It’s true that this code becomes slightly less intuitive (especially if you’re only getting used to logical properties),
...
Positioning in CSS surely is complicated, especially if positions of the elements
are determined by a page that can look like absolutely anything at all
