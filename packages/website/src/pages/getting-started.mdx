...
- **Use the [`output.page` option](/api#outputpage) to control whether highlights should be displayed at all.**
  To limit distractions on the page itself, you may want to start with only outputting issues
  to the console (`output: { page: false }`).
  Once the amount of issues is more manageable, you can enable highlights on the page again.
...
Accented is most effective when it outputs interactive highlights next to elements with accessibility issues.
That way, the issues are hard to ignore.

Still, issues are also printed to the console, and you may choose to not modify the DOM at all,
and instead opt for a console-only mode:
...
See [output](/api#output) for details.
