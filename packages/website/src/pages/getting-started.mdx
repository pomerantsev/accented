...
import screenshotDefaultColor from '~/pages/images/screenshot-default-color.png';
import screenshotGreen from '~/pages/images/screenshot-green.png';
...
Run it on the client side only. Accented doesn’t do anything on the server side.
...
- **Call once during initialization (the simple option).**
  Call `accented()` at any point during your app’s client-side initialization. That’s it.
