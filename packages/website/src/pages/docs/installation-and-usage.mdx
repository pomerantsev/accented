...
- Accented’s [browser support policy](/docs/browser-support-policy/) may be different from that of your application,
...
- **Use the [`context` option](/docs/api/#context) to limit scans to certain parts of the page.**
...
- **Use the [`axeOptions` option](/docs/api/#axeoptions) to include or exclude some axe rules.**
...
export function Accented({ children }) {
   ...
   * See https://www.accented.dev/docs/installation-and-usage/#run-in-development-mode
   */
  if (isDevelopment) {
    return (
...
Accented _may_ [negatively affect the performance](/docs/how-accented-works/#performance) of your application in development,
...
you can use the [`callback` option](/docs/api/#callback):
...
Use the [`throttle` option](/docs/api/#throttle)
...
See [`axeOptions` documentation](/docs/api/#axeoptions) for more details.
...
See full CSS API in the [Styling section](/docs/api/#styling) of the [API docs](/docs/api/).
