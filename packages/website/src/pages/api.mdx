...
- `scanContext`: nodes that got scanned (a subset of the full context that was passed as an option to Accented).
  The value can be either an array of nodes, or an object with `include` and `exclude` properties (if any nodes were excluded).
  <br/>
  **Note:** for a small number of rules whose success or failure depends on the element’s descendants (not just direct children),
  Accented always scans the full context to ensure its output is accurate.
  So technically, up to two scans may be run each time: one over `scanContext` (limited to the elements that recently changed),
  and the other over the full context passed to Accented (with a limited number of specific axe-core rules).
  `callback` will be called only once for these two scans (which Accented treats as a single pass).
...
- as interactive highlights on the page;
- as console messages.

You can use the `output` option if you’d like to only keep one or the other.

For example, you may only keep the console messages if you don’t want Accented to modify the DOM of your app (see [Console-only mode](/getting-started#console-only-mode)).
